...
class CommandStatusBuilderPart implements ObjectDefinitionBuilderPart {
	...
	public function apply (target:ObjectDefinition) : void {
		...
		factory = CommandStatusFlag.newFactory(property.name, manager, CommandStatus.EXECUTE, messageType, info.selector, info.order);
		target.addProcessorFactory(new MessageReceiverProcessorFactory(target, factory, info.config.context, info.scope));
		
		factory = CommandStatusFlag.newFactory(property.name, manager, CommandStatus.COMPLETE, messageType, info.selector, info.order);
		target.addProcessorFactory(new MessageReceiverProcessorFactory(target, factory, info.config.context, info.scope));
		
		factory = CommandStatusFlag.newFactory(property.name, manager, CommandStatus.ERROR, messageType, info.selector, info.order);
		target.addProcessorFactory(new MessageReceiverProcessorFactory(target, factory, info.config.context, info.scope));
		
		factory = CommandStatusFlag.newFactory(property.name, manager, CommandStatus.CANCEL, messageType, info.selector, info.order);
		target.addProcessorFactory(new MessageReceiverProcessorFactory(target, factory, info.config.context, info.scope));
