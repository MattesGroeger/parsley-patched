...
public class FlexLoggingXmlTagTest {
	...
	private static const config:XML = <objects 
		...
		>
		<log:target id="debugTarget" type="org.spicefactory.parsley.integration.logging.LogCounterTarget" level="debug">
			<log:filter>log.debug.*</log:filter>		
		</log:target>
		<log:target id="infoTarget" type="org.spicefactory.parsley.integration.logging.LogCounterTarget" level="info">
			<log:filter>log.info.*</log:filter>		
		</log:target>
		<log:target id="warnTarget" type="org.spicefactory.parsley.integration.logging.LogCounterTarget" level="warn">
			<log:filter>foo.*</log:filter>		
		</log:target>
		<log:target id="errorTarget" type="org.spicefactory.parsley.integration.logging.LogCounterTarget" level="error">
			<log:filter>log.error.*</log:filter>		
		</log:target>
		<log:target id="fatalTarget" type="org.spicefactory.parsley.integration.logging.LogCounterTarget" level="fatal">
	...
	public function logTargetConfig () : void {
		...
		XmlContextUtil.newContext(config);
		logAllLevels();
	}
