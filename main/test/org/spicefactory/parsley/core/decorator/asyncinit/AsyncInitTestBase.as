...
import org.spicefactory.parsley.core.decorator.asyncinit.model.SyncModel;
...
public class AsyncInitTestBase extends ContextTestBase {
	...
	public function testOrderedAsyncInit () : void {
		...
		checkState(context, true, false);
		checkObjectIds(context, ["asyncInitModel1", "asyncInitModel2", "asyncInitModel3"], AsyncInitModel);
		assertEquals("First model should be initialized", 1, AsyncInitModel.instances);
		assertEquals("Sync model should still be unitialized", 0, SyncModel.instanceCount);
		...
		checkState(context, true, false);
		assertEquals("Third model should now be initialized", 3, AsyncInitModel.instances);
		assertEquals("Sync model should still be unitialized", 0, SyncModel.instanceCount);
		...
		checkState(context, true, true);
		assertEquals("Sync model should now be intialized", 1, SyncModel.instanceCount);
	}
