...
public class ViewConfigurationHandler implements ViewRootHandler {
	...
	public function destroy () : void {
		...
		for each (var config:ViewConfiguration in activeConfigs) {
			if (config.lifecycle) config.lifecycle.stop();
		}
		activeConfigs = null;
	}
	...
	private function isAutoremove (target:Object, defaultMode:Boolean) : Boolean {
		if (!(target is DisplayObject)) return false;
		var info:ClassInfo = ClassInfo.forInstance(target, context.domain);
